...
export default function InvoiceForm({ form, customers, jobs = [], onSubmit, submitLabel }) {
    ...
    return (
        <form className="space-y-6 rounded-lg border border-slate-200 bg-white p-4" onSubmit={onSubmit}>
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="text-sm font-medium text-slate-700">Linked job<select className="mt-1 block w-full rounded-md border px-3 py-2" value={data.job_id || ''} onChange={(event) => setData('job_id', event.target.value)}><option value="">Standalone invoice</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.title} - {job.customer?.display_name}</option>)}</select></label>
